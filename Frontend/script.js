const apiGateway = "https://q9dd28zox4.execute-api.us-east-1.amazonaws.com/trial";
...
async function getTodo() {
  ...
  const apiUrl = `${apiGateway}/get?id=${todoId}`;
  try {
    const response = await fetch(apiUrl, {
      method: 'GET',
      headers: {
...
function createUpdateButtons(todoId, todoName, todoDescription, todoStatus) {
  ...
  const statusButtonText = todoStatus === 'COMPLETED' ? 'Uncomplete' : 'Complete';
  createButton(statusButtonText, 'update-status-button', () => {
...
async function deleteTodo(id) {
  try {
    const response = await fetch(`${apiGateway}/delete?id=${id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json'
      }
...
async function updateName(id, newName, description) {
  try {
    const response = await fetch(`${apiGateway}/update?id=${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function updateStatus(id, currentStatus) {
  ...
  try {
    const response = await fetch(`${apiGateway}/complete?id=${id}&status=${newStatus}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    if (response.ok) {
      document.getElementById('todo-status-display').textContent = newStatus.toUpperCase();
      createUpdateButtons(id, document.getElementById('todo-name-display').textContent, document.getElementById('todo-description-display').textContent, newStatus);
    } else {
      alert('Update failed. Please try again.');
    }
  } catch (error) {
...
async function updateStatusOnList(id) {
  try {
    const response = await fetch(`${apiGateway}/complete?id=${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      }
